...
type ApiObserveDashboardProps = {
  params: {
    agent?: string;
    evalCase?: string;
    session?: string;
  };
  filterQueryParams: ApiFilterSearchInput & { from: string; to: string };
  preserveDateRange: Pick<ApiFilterSearchInput, "from" | "to">;
};
...
export async function ApiObserveDashboard({
  params,
  filterQueryParams,
  preserveDateRange,
}: ApiObserveDashboardProps) {
  const filters = {
    agent: params.agent,
  ...
  const [summary, sessions, agents, evalCases] = await Promise.all([
    ...
    listDistinctAgents({ from: filterQueryParams.from, to: filterQueryParams.to }),
    listDistinctEvalCases({ from: filterQueryParams.from, to: filterQueryParams.to }),
  ]);

  const filterQuery = buildApiFilterQuery(filterQueryParams);
  const hasActiveFilters = hasActiveApiFilters(filterQueryParams);
  const hasData = summary.apiRequestCount > 0 || (summary.discoveryHits ?? 0) > 0;
      ...
      >
        ...
        <div className="shrink-0 space-y-1 text-ui">
          ...
          <div className="flex items-center gap-1.5">
            <ObserveApplyButton />
            {hasActiveFilters ? (
              <ObserveClearFiltersButton preserveDateRange={preserveDateRange} scope="api" />
            ) : null}
          </div>
        </div>
      </ObserveFiltersForm>
                ...
                {sessions.map((session) => (
                  <tr key={session.sessionId} className="hover:bg-zinc-50 dark:hover:bg-zinc-950/40">
                    <td className="px-4 py-3">
                      <Link
                        href={`/observe/api/sessions/${encodeURIComponent(session.sessionId)}${filterQuery}`}
